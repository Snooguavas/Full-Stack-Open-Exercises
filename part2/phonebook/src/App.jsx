...
const Notification = ({ text, color }) => {
  ...
  const App = () => {
    ...
    ? persons
    : persons.filter(person => person.name.toLowerCase().includes(toSearch.toLowerCase()))
      ...
      for (let person of persons) {
        if (person.name === newPerson.name) {
          if (person.number === newPerson.number){
            createNotification(`${person.name} is already added to phone book!`, 'red', 2000)
            return;
          }
          if (window.confirm(`${person.name} is already added to phone book! replace old number with new one?`)) {
    ...
    return (
      ...
      <AddForm handleSubmit={addPerson} handleNameChange={handleNameChange}
      handleNumberChange={handleNumberChange} newPerson={newPerson} />
      ...
      <h2>Numbers</h2>
      <ul>
      {peopleToShow.map(person => <Person person={person} key={person.id} deletePerson={deletePerson}/>)}
      </ul>
      </div>
      </div>
      </div>
